...
### 이 문제를 풀고 난 점
* 맞긴 맞았는데 너무 찝찝했다. 내 코드는 긴데 분명히 더 짧은 방법이 있다고 생각한 나는 생각하지 못하고 다른 사람의 정답을 봤는데
* ```python
  S = input()
  i = int(input())
  print(S[i - 1])
  ```
* 그동안 너무 map함수에 주구 장창 써서 문자열도 그렇게 생각했는데 map함수 이용할 필요 없이 그냥 input()만 사용해도 된다는 걸 보고 살짝 내가 너무 멍청했고 바보같다는 생각이 들었다
