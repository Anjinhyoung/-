...
### 이 문제를 풀고난 점
 * 이 문제 처음 풀었을 때 꽤 고생했던 기억이 난다. 너무 어려워가지고
 * '분'은 괜찮은데 '시'를 구하는 게 애먹었다.
 * 문제에서 요리하는 시간은 최대 '1000'분이다.
 * 즉 극단적으로 23시 59분에 1000분을 넘어가도 이틀은 넘어가지 않는다. -> 내 개인적으로 이 문제에 가장 큰 핵심이라고 생각한다.
 * 이 핵심을 생각하기까지 한 20분 정도 걸렸던 거 같다.
 * 이런 많은 경우가 있을 때는 한 번 극단적인 경우도 생각해보자
