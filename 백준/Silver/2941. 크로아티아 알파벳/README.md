...
# 이 문제를 풀고난 후
* 'replace'방법을 제대로 알았다.  ** c.replace('a','b') ** 이렇게 사용하면 안 된다.
* 왜냐하면 새로운 문자열을 반환하지만, c 변수에는 할당되지 않음
* 그러므로 **c = c.replace("a", "x") ** 이런식으로
* 새로운 문자열을 변수 c에 할당하여 값을 변경해줘야 한다.
* 참고로 내가 쓴 답은 별로 좋은 답이 아니다.
* 파이썬은 루프에서 컬렉션을 변경하는 경우에도 내부적으로 안전하게 처리하도록 설계되어있어서
* 의도치 않는 에러가 발생할 수 있다.
